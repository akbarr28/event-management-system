import pytest
from datetime import datetime, timedelta
from decimal import Decimal

from src.domain.booking.value_objects.booking_id import BookingId
from src.domain.booking.value_objects.booking_status import BookingStatus
from src.domain.booking.value_objects.customer_id import CustomerId
from src.domain.event.value_objects.event_status import EventStatus
from src.domain.refund.aggregates.refund import Refund
from src.domain.refund.domain_events.refund_approved import RefundApproved
from src.domain.refund.value_objects.refund_status import RefundStatus
from src.domain.shared.exceptions.domain_exception import DomainException
from src.domain.shared.value_objects.money import Money


# Fixtures 

@pytest.fixture
def now():
    return datetime.utcnow()


@pytest.fixture
def requested_refund(now):
    """Refund yang sudah berstatus REQUESTED."""
    refund = Refund.request(
        booking_id=BookingId.generate(),
        customer_id=CustomerId.generate(),
        amount=Money(Decimal("150000"), "IDR"),
        booking_status=BookingStatus.PAID,
        event_status=EventStatus.PUBLISHED,
        has_checked_in_tickets=False,
        refund_deadline=now + timedelta(days=7),
        now=now,
    )
    refund.pull_domain_events()
    return refund


# Happy Path 

def test_approve_refund_success(requested_refund):
    """Refund berhasil disetujui."""
    requested_refund.approve()

    assert requested_refund.status == RefundStatus.APPROVED


def test_approve_refund_raises_domain_event(requested_refund):
    """Sistem harus raise domain event RefundApproved."""
    requested_refund.approve()
    domain_events = requested_refund.pull_domain_events()

    assert len(domain_events) == 1
    assert isinstance(domain_events[0], RefundApproved)
    assert domain_events[0].refund_id == requested_refund.id
    assert domain_events[0].booking_id == requested_refund.booking_id


def test_approve_refund_does_not_change_amount(requested_refund):
    """Amount refund tidak berubah setelah disetujui."""
    original_amount = requested_refund.amount
    requested_refund.approve()

    assert requested_refund.amount == original_amount


def test_approve_refund_rejection_reason_remains_none(requested_refund):
    """Approve tidak mengisi rejection reason."""
    requested_refund.approve()

    assert requested_refund.rejection_reason is None


# Unhappy Path 

def test_approve_refund_fails_when_already_approved(requested_refund):
    """Refund yang sudah APPROVED tidak bisa disetujui lagi."""
    requested_refund.approve()
    requested_refund.pull_domain_events()

    with pytest.raises(DomainException) as exc_info:
        requested_refund.approve()

    assert "requested" in str(exc_info.value).lower()


def test_approve_refund_fails_when_rejected(requested_refund):
    """Refund yang sudah REJECTED tidak bisa disetujui."""
    requested_refund.status = RefundStatus.REJECTED

    with pytest.raises(DomainException) as exc_info:
        requested_refund.approve()

    assert "requested" in str(exc_info.value).lower()


def test_approve_refund_fails_when_paid_out(requested_refund):
    """Refund yang sudah PAID_OUT tidak bisa disetujui."""
    requested_refund.status = RefundStatus.PAID_OUT

    with pytest.raises(DomainException) as exc_info:
        requested_refund.approve()

    assert "requested" in str(exc_info.value).lower()


# Mark Refunded di Booking 

def test_booking_mark_refunded_success():
    """Booking berstatus PAID berhasil ditandai sebagai REFUNDED."""
    from src.domain.booking.aggregates.booking import Booking
    from src.domain.event.value_objects.ticket_category_id import TicketCategoryId
    from src.domain.event.value_objects.event_id import EventId

    now = datetime.utcnow()
    booking = Booking.create(
        customer_id=CustomerId.generate(),
        event_id=EventId.generate(),
        ticket_category_id=TicketCategoryId.generate(),
        quantity=2,
        unit_price=Money(Decimal("150000"), "IDR"),
        now=now,
    )
    booking.pay(
        payment_amount=Money(Decimal("300000"), "IDR"),
        now=now,
    )

    booking.mark_refunded()

    assert booking.status == BookingStatus.REFUNDED


def test_booking_mark_refunded_fails_when_not_paid():
    """Hanya booking PAID yang bisa ditandai REFUNDED."""
    from src.domain.booking.aggregates.booking import Booking
    from src.domain.event.value_objects.ticket_category_id import TicketCategoryId
    from src.domain.event.value_objects.event_id import EventId

    now = datetime.utcnow()
    booking = Booking.create(
        customer_id=CustomerId.generate(),
        event_id=EventId.generate(),
        ticket_category_id=TicketCategoryId.generate(),
        quantity=1,
        unit_price=Money(Decimal("150000"), "IDR"),
        now=now,
    )
    # Booking masih PENDING_PAYMENT

    with pytest.raises(DomainException) as exc_info:
        booking.mark_refunded()

    assert "paid" in str(exc_info.value).lower()