...
def upgrade() -> None:
    op.execute("CREATE TYPE event_status AS ENUM ('DRAFT', 'PUBLISHED', 'CANCELLED', 'COMPLETED')")
    op.execute("CREATE TYPE ticket_category_status AS ENUM ('ACTIVE', 'INACTIVE')")
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("DRAFT", "PUBLISHED", "CANCELLED", "COMPLETED", name="event_status", create_type=False),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("ACTIVE", "INACTIVE", name="ticket_category_status", create_type=False),
            nullable=False,
...
def downgrade() -> None:
    ...
    op.execute("DROP TYPE ticket_category_status")
